...
class Produit{
    ...
    static function create($ref,$nom,$prix,$local,$url_img,$id_cat,$qte){
        if($url_img != null){
            requete("INSERT INTO _produit (reference,nom,prix,quantite,url_img,est_local,id_categorie) 
            VALUES (:reference,:nom,:prix,:quantite,:url_img,:est_local,:id_categorie)",
            [":reference"=>$ref,":nom"=>$nom,":prix"=>$prix,":quantite"=>$qte,":url_img"=>$url_img,
            ":est_local"=>$local,":id_categorie"=>$id_cat]);
        }
        else{
            requete("INSERT INTO _produit (reference,nom,prix,quantite,est_local,id_categorie) 
            VALUES (:reference,:nom,:prix,:quantite,:est_local,:id_categorie)",
            [":reference"=>$ref,":nom"=>$nom,":prix"=>$prix,":quantite"=>$qte,":est_local"=>$local,
            ":id_categorie"=>$id_cat]);
